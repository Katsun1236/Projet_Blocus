-- Migration: Schéma original complet pour connexion universelle
-- Date: 2025-01-05
-- Description: Appliquer le schéma original de schema.sql pour garantir la connexion de tous
...
-- Trigger automatique pour les nouveaux utilisateurs
...
-- Données initiales pour les groupes communautaires
...
SELECT id, NOW()
FROM auth.users
WHERE id NOT IN (SELECT user_id FROM public.onboarding);
