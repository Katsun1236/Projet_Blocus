import { auth, db, functions } from '../../core/services/firebase/index.js';
...
async function generateQuiz() {
    ...
    if (source === 'topic') {
        ...
        if (!topic) return showToast("Veuillez décrire le sujet.", "error");
        if (!title) title = topic.substring(0, 30) + "...";
    }
    else if (source === 'synthesis') {
        const synthId = ui.synthesisSelect.value;
        if (!synthId) return showToast("Veuillez sélectionner une synthèse.", "error");
        ...
        if (!fileId) return showToast("Veuillez sélectionner un cours.", "error");
    ...
    try {
        ...
        showToast("Quiz prêt !", "success");
        startQuiz(quizData);
        closeModal();

    } catch (error) {
        console.error("Erreur Génération:", error);
        showToast("Erreur IA : " + (error.message || "Réessayez plus tard."), "error");
    } finally {
        isGenerating = false;
