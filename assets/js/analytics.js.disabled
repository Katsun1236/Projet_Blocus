...
async function calculateStats(userData) {
    ...
    // Stats de base depuis le profil
    const stats = {
        totalTime: 0,
        quizzesCompleted: 0,
        successRate: 0,
    ...
    // Estimer le temps de révision (basé sur activité)
...
function estimateStudyTime(quizzes, synthesis, courses) {
    ...
    return Math.round(totalMinutes / 60 * 10) / 10; // Heures avec 1 décimale
}
